import { X, RefreshCw, History, Building, Layers } from 'lucide-react';
import { Button } from '@shared/ui';
import type { WarehouseSyncLog } from '../../../types/warehouse.types';
import { useEffect } from 'react';

interface WarehouseSyncHistoryDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  logs: WarehouseSyncLog[];
  isLoading: boolean;
  onRefresh: () => void;
}
...
export default function WarehouseSyncHistoryDrawer({
  isOpen,
  onClose,
  logs,
  isLoading,
  onRefresh,
}: WarehouseSyncHistoryDrawerProps) {
            ...
            <div className="h-full flex flex-col items-center justify-center gap-3.5 text-center px-6">
              ...
              <div>
                ...
                <p className="text-xs font-medium text-slate-400 mt-1 max-w-[240px] mx-auto leading-relaxed">
                  Hệ thống chưa ghi nhận lần đồng bộ KiotViet nào thành công trước đó.
                </p>
              </div>
            </div>
          ) : (
              ...
              <div className="space-y-6">
                {logs.map((log) => (
                  <div key={log.id} className="relative group">
                    {/* Bullet marker */}
                    <div className="absolute -left-[22px] top-1.5 h-3.5 w-3.5 rounded-full border-2 border-white bg-emerald-500 shadow-xs ring-4 ring-emerald-50 transition-transform group-hover:scale-110" />
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-2 flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex gap-1.5 text-xs font-extrabold">
                              <span className="text-emerald-700">+{log.branchesAdded}</span>
                              <span className="text-slate-400 font-medium">/</span>
                              <span className="text-slate-650">~{log.branchesUpdated}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-2 flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex gap-1.5 text-xs font-extrabold">
                              <span className="text-blue-700">+{log.productsAdded}</span>
                              <span className="text-slate-400 font-medium">/</span>
                              <span className="text-slate-655">~{log.productsUpdated}</span>
                            </div>
                          </div>
                        </div>
                      </div>
