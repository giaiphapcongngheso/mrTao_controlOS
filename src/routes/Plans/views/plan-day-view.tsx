...
import { Zap, UserCheck, AlertTriangle, Link2, ArrowRight, StickyNote } from 'lucide-react';
...
import { filterPlansByLevel, formatCurrencyVN } from '../constants/plan-utils';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-4">
        ...
        <Card className="border border-slate-100/80 p-0 shadow-2xs bg-white rounded-2xl py-4">
          <CardContent className="px-4 p-0">
            <h4 className="text-sm font-black text-slate-700 mb-3">Kế hoạch theo khung giờ</h4>
            {timeSlots.length > 0 ? (
              ...
              <div className="py-8 text-center space-y-2">
                ...
                <p className="text-xs font-semibold text-slate-350">Thêm các khung giờ làm việc để theo dõi tiến độ trong ngày.</p>
              </div>
            )}
                ...
                <React.Fragment key={item.label}>
                  <div className="flex items-center gap-2 min-w-[120px]">
                    ...
                    <div>
                      ...
                      <span className="text-xs font-semibold text-slate-450 mt-0.5 block">{item.desc}</span>
                    </div>
                  </div>
              ...
              {quickNotes.length > 0 ? (
                <ul className="space-y-1">
                  {quickNotes.map((note, i) => (
                    <li key={i} className="text-xs font-semibold text-slate-650 bg-amber-50/50 border border-amber-100/50 px-2.5 py-1.5 rounded-lg leading-normal">
                      {note}
                    </li>
                  ))}
                </ul>
